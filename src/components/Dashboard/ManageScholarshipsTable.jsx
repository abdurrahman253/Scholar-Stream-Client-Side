...
const ManageScholarships = () => {
 ...
 const updateMutation = useMutation({
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50">
        <div className="text-center max-w-md mx-auto p-8 bg-white rounded-3xl shadow-2xl">
          ...
          <p className="text-gray-600 mb-4">{error?.message || 'Failed to fetch scholarships'}</p>
          <button
            onClick={() => queryClient.invalidateQueries(['all-scholarships-admin'])}
            className="px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold rounded-xl hover:shadow-xl transition-all"
          >
            Try Again
          </button>
        </div>
  ...
  // Check if scholarships is an array
  if (!Array.isArray(scholarships)) {
    console.error('Scholarships is not an array:', scholarships);
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50">
        <div className="text-center max-w-md mx-auto p-8 bg-white rounded-3xl shadow-2xl">
          <div className="text-6xl mb-4">⚠️</div>
          <h3 className="text-2xl font-bold text-gray-800 mb-2">Data Format Error</h3>
          <p className="text-gray-600 mb-4">Received invalid data format from server</p>
          <button
            onClick={() => queryClient.invalidateQueries(['all-scholarships-admin'])}
            className="px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold rounded-xl hover:shadow-xl transition-all"
          >
